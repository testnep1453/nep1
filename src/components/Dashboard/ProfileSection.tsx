        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          {Object.keys(BADGE_INFO).map((badgeId) => {
            ...
            return (
              <div
                key={badgeId}
                className={`group relative p-2 sm:p-3 rounded-xl border-2 transition-all cursor-help flex flex-col items-center justify-center ${
                  unlocked
                    ? 'bg-[#1a1d2e] border-[#ff9f43] shadow-lg shadow-[#ff9f43]/30'
                    : 'bg-[#1a1d2e]/30 border-white/10 opacity-40 hover:opacity-80'
                }`}
              >
                <div className="text-2xl sm:text-3xl text-center mb-1 transition-transform group-hover:scale-110">
                  {badge.emoji}
                </div>
                <div className="text-[10px] sm:text-xs text-center text-white/70 font-semibold">
                  {badge.name}
                </div>
