...
export const UnifiedDashboard = ({
  ...
  // Feedback gruplama (ders tarihine göre)
  ...
  return (
    <div className="h-[100dvh] bg-[#050505] text-white flex flex-col md:flex-row font-['Rajdhani',sans-serif] selection:bg-[#39FF14]/30 overflow-hidden">
      ...
      <OperationDrawer
        ...
        zoomLink={lesson?.zoomLink || LESSON_CONFIG.zoomLink}
      />

      {/* Feedback Form */}
      {showFeedback && (
        <FeedbackForm
          lessonDate={autoZoomState.lessonDate}
          studentId={student.id}
          onClose={() => {
            setShowFeedback(false);
            sessionStorage.setItem(`feedback_${autoZoomState.lessonDate}`, 'true');
          }}
        />
      )}
      ...
      <div className="md:hidden sticky top-0 z-30 bg-[#0A1128] border-b border-[#39FF14]/20 flex items-center justify-between px-4 py-3">
        <button
          ...
          className="text-[#00F0FF] p-2 min-w-[44px] min-h-[44px] flex items-center justify-center"
        >
          <Icons.Swords />
        </button>
        <img
            ...
            {tabConfig.map(tab => (
              <button
                ...
                className={`flex items-center gap-3 w-full p-3 rounded-md transition-all min-h-[48px] ${
                  activeTab === tab.id
                    ? (isAdmin ? 'bg-[#39FF14]/10 text-[#39FF14]' : 'bg-[#00F0FF]/10 text-[#00F0FF]')
          ...
          {tabConfig.map(tab => (
            <button
              ...
              className={`flex items-center gap-3 w-full p-3 rounded-md transition-all ${
                activeTab === tab.id
                  ? (isAdmin ? 'bg-[#39FF14]/10 text-[#39FF14] border-l-4 border-[#39FF14]' : 'bg-[#00F0FF]/10 text-[#00F0FF] border-l-4 border-[#00F0FF]')
            ...
            <div className="space-y-6 sm:space-y-8 animate-fade-in">
                ...
                <div className="space-y-4 max-w-xl mx-auto">
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="flex-1 bg-[#F5D32E]/20 hover:bg-[#F5D32E] text-[#F5D32E] hover:text-black border border-[#F5D32E] py-3 font-bold transition-all uppercase tracking-widest rounded min-h-[48px]"
                    >
                      Fragmanı Kaydet
                    </button>
                    {trailer && trailer.isActive && (
                      <button
                        ...
                        className="bg-[#FF4500]/20 hover:bg-[#FF4500] text-[#FF4500] hover:text-black border border-[#FF4500] py-3 px-6 font-bold transition-all uppercase tracking-widest rounded min-h-[48px]"
                      >
                        Kaldır
                      </button>
                    )}
              ...
              {trailer && trailer.isActive && trailer.youtubeId && (
                <div className="bg-[#0A1128]/80 border border-[#FF4500]/30 p-6 sm:p-8 clip-path-diagonal">
                  <h3 className="text-lg sm:text-xl font-bold text-[#FF4500] mb-6 uppercase tracking-wider text-center">
                    📺 FRAGMAN ÖNİZLEME
                  </h3>
              ...
              <div className="bg-[#0A1128]/80 border border-[#00F0FF]/30 p-4 sm:p-6 clip-path-diagonal relative">
                ...
                <form onSubmit={handleSendMessage}>
                  <textarea
                    ...
                    placeholder="Mesajınızı yazın..." required rows={4}
                  ...
                  />
                  <button type="submit"
                    className="w-full bg-[#00F0FF]/20 hover:bg-[#00F0FF] text-[#00F0FF] hover:text-black border border-[#00F0FF] py-3 font-bold transition-all uppercase tracking-widest flex items-center justify-center gap-2 rounded min-h-[48px]">
                    <Icons.Message /> GÖNDER
                  </button>
                </form>
              </div>
            </div>
          ...
          {isAdmin && activeTab === 'geribildirim' && (
            <div className="space-y-6 animate-fade-in">
              {feedbackLoading ? (
                ...
                Object.entries(groupedFeedback).map(([date, entries]) => (
                  <div key={date} className="bg-[#0A1128]/80 border border-[#6358cc]/30 rounded-lg overflow-hidden">
                    <div className="bg-[#6358cc]/10 px-4 sm:px-6 py-3 border-b border-[#6358cc]/20">
                      <h3 className="text-[#8b7fd8] font-bold text-sm uppercase tracking-wider">
                        📅 {formatLessonDate(date)} — {entries.length} geri bildirim
                      </h3>
                      <div className="flex items-center gap-4 mt-1">
                        <span className="text-[#FF9F43] text-xs font-mono">
                          Ort. Puan: {(entries.reduce((sum, f) => sum + f.rating, 0) / entries.length).toFixed(1)} ⭐
                        </span>
                      </div>
                    </div>
                        ...
                        <div key={fb.id} className="p-4 sm:p-6 hover:bg-white/5 transition-colors">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex gap-1">
                              {[1, 2, 3, 4, 5].map(s => (
                                <span key={s} className={`text-lg ${s <= fb.rating ? '' : 'opacity-20'}`}>⭐</span>
                              ))}
                            </div>
      ...
      {/* Mobil Alt Bar — Öğrenci için hızlı erişim */}
      {!isAdmin && (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0A1128] border-t border-[#00F0FF]/20 z-20 flex items-center justify-around px-2 py-2 safe-area-bottom">
          <button
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-all min-w-[60px] min-h-[56px] ${
              activeTab === 'genel' ? 'text-[#00F0FF]' : 'text-gray-500'
            }`}
          >
            <Icons.Home />
          ...
          <button
            onClick={() => setDrawerOpen(true)}
            className="flex flex-col items-center gap-1 p-2 rounded-lg bg-[#6358cc]/20 text-[#8b7fd8] min-w-[60px] min-h-[56px]"
          >
            <Icons.Swords />
            <span className="text-[10px] font-bold">OPERASYON</span>
          </button>
        </div>
      )}
    </div>
