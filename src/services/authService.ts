...
export const getStudentById = async (id: string): Promise<Student | null> => {
  try {
    ...
    return {
      id: student.id, name: student.name, nickname: student.display_name || student.displayName || student.nickname || 'AJAN',
      email: student.email, xp: student.xp ?? 0, level: student.level ?? 1, badges: student.badges || [],
      avatar: student.avatar || 'hero_1', lastSeen: new Date(student.last_seen || student.lastSeen || Date.now()).getTime(),
      attendanceHistory: student.attendance_history || student.attendanceHistory || [], streak: student.streak ?? 0,
    };
  } catch { return null; }
};
...
export const upsertStudent = async (student: Student): Promise<boolean> => {
  try {
    const { error } = await supabase.from('students').upsert({
      ...
      "streak": student.streak, "badges": student.badges, "attendance_history": student.attendanceHistory || [],
