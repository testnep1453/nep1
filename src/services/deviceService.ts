...
export const registerDevice = async (studentId: string): Promise<{ isNew: boolean; needsApproval: boolean }> => {
  ...
  try {
    const { data: student } = await supabase.from('students').select('devices, primaryDeviceFingerprint').eq('studentId', studentId).single();
    ...
    if (existingDevice) {
      ...
      await supabase.from('students').update({ devices: updatedDevices }).eq('studentId', studentId);
    ...
    if (!primaryFingerprint) {
      await supabase.from('students').update({ devices: [...devices, newDevice], primaryDeviceFingerprint: device.fingerprint }).eq('studentId', studentId);
      return { isNew: true, needsApproval: false };
    }

    await supabase.from('students').update({ devices: [...devices, newDevice] }).eq('studentId', studentId);
